...
class TestAvocadoParams(unittest.TestCase):
    ...
    def test_unhashable(self):
        ...
        self.assertEqual(
            self.params1.get("unique1", "/ch0/ch0.1/ch0.1.1/ch0.1.1.1/", ["bar"]),
            "unique1",
        )
    ...
    def test_get_abs_path(self):
        ...
        self.assertEqual(self.params1.get("unique1", "/ch0/*", "ccc"), "unique1")
        self.assertEqual(self.params2.get("unique1", "/ch0/*", "ddd"), "unique1-2")
        self.assertEqual(self.params1.get("unique3", "/ch0/*", "eee"), "unique3")
        # unique3 is not in self.params2
        self.assertEqual(self.params2.get("unique3", "/ch0/*", "fff"), "fff")
        # Use the leaf
        self.assertEqual(
            self.params1.get("unique1", "/ch0/ch0.1/ch0.1.1/ch0.1.1.1/", "ggg"),
            "unique1",
        )
        # '/ch0/ch0.1/ch0.1.1/' is in the tree, but not in current variant
        self.assertEqual(
            self.params2.get("unique1", "/ch0/ch0.1/ch0.1.1/ch0.1.1.1/", "hhh"), "hhh"
        )

    def test_get_greedy_path(self):
        self.assertEqual(
            self.params1.get("unique1", "/*/*/*/ch0.1.1.1/", 111), "unique1"
        )
        # evaluated from right
        self.assertEqual(self.params1.get("unique1", "/*/*/ch0.1.1.1/", 222), "unique1")
        # path too long so can't match from right
        self.assertEqual(self.params1.get("unique1", "/*/*/*/*/ch0.1.1.1/", 333), 333)
        self.assertEqual(
            self.params1.get("unique1", "/ch*/c*1/*0*/*1/", 444), "unique1"
        )
        # '/ch0/ch0.1/ch0.1.1/' is in the tree, but not in current variant
        self.assertEqual(self.params2.get("unique1", "/ch*/c*1/*0*/*1/", 555), 555)
        self.assertEqual(self.params2.get("unique1", "/ch*/c*1/*", 666), "unique1-2")
        self.assertEqual(self.params1.get("unique4", "/ch1*/*", 777), "other_unique")
        self.assertEqual(self.params1.get("unique2", "/ch1*/*", 888), "unique2")
    ...
    def test_get_rel_path(self):
        ...
        # Use the leaf
        self.assertEqual(self.params1.get("unique1", "*/ch0.1.1.1/", "nnn"), "unique1")
        # '/ch0/ch0.1/ch0.1.1/' is in the tree, but not in current variant
        self.assertEqual(self.params2.get("unique1", "*/ch0.1.1.1/", "ooo"), "ooo")
    ...
    def test_get_clashes(self):
        ...
        self.assertEqual(self.params2.get("clash3", default="nnn"), "also equal")
...
class TestInternalFilters(unittest.TestCase):
    ...
    def test_basic(self):
        ...
        self.assertTrue(
            self.check_scenario(("foo", [], []), ("bar", [], ["/nonexists"]))
        )
        self.assertTrue(self.check_scenario(("1/foo", [], []), ("1/bar", ["/1"], [])))
        # The /1/foo is not the same parent as /2/bar filter
        self.assertTrue(
            self.check_scenario(("1/foo", [], []), ("2/bar", ["/2/bar"], []))
        )
        self.assertFalse(
            self.check_scenario(
                ("/1/foo", ["/1/bar"], []),
        ...
        self.assertFalse(
            self.check_scenario(("1/foo", ["/1/foo"], []), ("1/bar", ["/1"], []))
        )
        # ... unless you allow both of them
        self.assertTrue(
            self.check_scenario(
                ("1/foo", ["/1/foo", "/1/bar"], []), ("1/bar", ["/1"], [])
            )
        )
        ...
        self.assertTrue(
            self.check_scenario(("/1/1/foo", ["/1/1/foo", "/1", "/1/1"], []))
        )
        # Three levels
        self.assertTrue(
            self.check_scenario(
...
class TestCreateFromYaml(unittest.TestCase):
    def test_normalize_path(self):
        ...
        self.assertEqual(
            yaml_to_mux._normalize_path("path"), "path/"
    ...
    def test_handle_control_tag_using(self):
        # pylint: disable=W0212
        using = yaml_to_mux._handle_control_tag_using(
            "fake_path", "name", False, "/using/path/"
        )
...
class TestFingerprint(unittest.TestCase):
    def test_fingerprint(self):
        ...
        self.assertNotEqual(
            node1b_env_orig.fingerprint(), node1b_env_origb.fingerprint()
        )
