...
class LoaderTest(unittest.TestCase):
    def _check_discovery(self, exps, tests):
        ...
        try:
            for exp, tst in zip(exps, tests):
                ...
                self.assertEqual(
                    os.path.abspath(tst[1]["name"]), os.path.abspath(exp[1])
                )
    ...
    def test_multiple_methods(self):
        avocado_multiple_tests = script.TemporaryScript(
            "multipletests.py",
        ...
        suite = self.loader.discover(
            avocado_multiple_tests.path, loader.DiscoverMode.ALL
        )
        self.assertEqual(len(suite), 2)
        # Try to load only some of the tests
        suite = self.loader.discover(
            avocado_multiple_tests.path + ":MultipleMethods.testTwo",
        ...
        # Load using regex
        suite = self.loader.discover(
            avocado_multiple_tests.path + ":.*_one", loader.DiscoverMode.ALL
        )
        self.assertEqual(len(suite), 1)
        self.assertEqual(suite[0][1]["methodName"], "test_one")
        # Load booth
        suite = self.loader.discover(
            avocado_multiple_tests.path + ":test.*", loader.DiscoverMode.ALL
        )
        self.assertEqual(len(suite), 2)
        # Load none should return no tests
        self.assertTrue(
            not self.loader.discover(
                avocado_multiple_tests.path + ":no_match", loader.DiscoverMode.ALL
            )
        )
    ...
    def test_multiple_methods_same_name(self):
        ...
        # Try to load only some of the tests
        suite = self.loader.discover(
            avocado_multiple_tests.path + ":MultipleMethods.test",
            loader.DiscoverMode.ALL,
        )
        self.assertEqual(len(suite), 1)
    ...
    def test_load_foreign(self):
        avocado_pass_test = script.TemporaryScript(
            "foreign.py", AVOCADO_FOREIGN_TAGGED_ENABLE, "avocado_loader_unittest"
        )
    ...
    def test_imports(self):
        ...
        tests = self.loader.discover(path)
        exps = [
            ("Test1", "selftests/.data/safeloader/data/imports.py:Test1.test"),
    ...
    def test_double_import(self):
        ...
        tests = self.loader.discover(path)
        exps = [
            ("Test1", "selftests/.data/safeloader/data/double_import.py:Test1.test1"),
